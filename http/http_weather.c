...
int http_client_request_async(async_context_t *context, WEATHER_HTTP_REQUEST_T *req) {
    #if LWIP_ALTCP
    ...
    if (ret != ERR_OK) {
        if (DEBUG_LEVEL >= DEBUG_NORMAL) {
            fprintf(stderr, "http request failed: %d", ret);
        }
    }
    return ret;
}

int http_client_request_sync(async_context_t *context, WEATHER_HTTP_REQUEST_T *req) {
    assert(req);
...
weather_t get_weather() {
    WEATHER_HTTP_REQUEST_T req = {0};
    req.hostname = "api.openweathermap.org";
    char url_buf[100];
    sprintf(url_buf, "/data/2.5/weather?lat=%s&lon=%s&appid=%s&mode=xml&units=imperial", LATITUDE_STR, LONGITUDE_STR, OPENWEATHER_API_KEY);
    if (DEBUG_LEVEL >= DEBUG_HIGH) {
        printf("Weather HTTP Request URL: \"%s\"", url_buf);
    }
    req.url = url_buf;
